...
function ownVenueSections(venue, dynamicTypes, dynamicElements) {
  return [
    ...
    },
    {
      ...
      collapsible: true,
      fields: [
        TM('Decor Time', 'decor_time', false, { requiredWhen: (f) => f.status === 'Confirmed' }),
        TM('Varmala Time', 'varmala_time', false),
        TM('Pheras Time', 'pheras_time', false, { inlineCheckbox: { key: 'pheras_next_day', label: '+1' } }),
        S('Decor Status', 'decor_status', DECOR_STATUSES, false, { requiredWhen: (f) => f.status === 'Confirmed' }),
        S('Decor Category', 'function_category', FUNCTION_CATEGORIES, false, { disabledWhen: decorOutdoor, helperText: decorOutdoorHelper, requiredWhen: (f) => f.status === 'Confirmed' }),
        T('Pending Payment — Decor %', 'payment_remaining_decor', false, {
          filterFn: percentFilter, filterError: 'Only numbers 0-100',
          suffix: '%', inputMode: 'numeric',
          disabledWhen: decorOutdoor, helperText: decorOutdoorHelper,
          requiredWhen: (f) => f.status === 'Confirmed',
        }),
    ...
    },
    {
      ...
      collapsible: true,
      fields: [
        S('Entertainment Status', 'entertainment_status', ENTERTAINMENT_STATUSES, false, { requiredWhen: (f) => f.status === 'Confirmed' }),
